<#
.SYNOPSIS
    Installs binoc on Windows.

.DESCRIPTION
    Downloads the Velopack installer (Binoc-win-Setup.exe) for a GitHub release, verifies it against the
    ...
    rest of the script mis-parses.

.PARAMETER Version
    Install a specific version (e.g. 0.1.0) instead of the latest release. A leading "v" is fine.
    Also reads $env:BINOC_VERSION, which is how you pin through the piped one-liner:
        $env:BINOC_VERSION = '0.1.0'; irm .../install.ps1 | iex

.PARAMETER Repo
    owner/name of the GitHub repository to install from. Defaults to ArcticGizmo/binoc; override for a
    fork. Also reads $env:BINOC_REPO.
...
.EXAMPLE
    ...
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/ArcticGizmo/binoc/main/install.ps1))) -Version 0.1.0
#>
#Requires -Version 5.1
param(
    [string] $Version = $env:BINOC_VERSION,
    [string] $Repo    = $(if ($env:BINOC_REPO) { $env:BINOC_REPO } else { 'ArcticGizmo/binoc' })
)
...
function Install-Binoc {
    [CmdletBinding()]
    param([string] $Version, [string] $Repo)

    $ErrorActionPreference = 'Stop'

    $SetupAsset = 'Binoc-win-Setup.exe'
    $SumsAsset  = 'SHA256SUMS.txt'

    # --- Preflight -------------------------------------------------------------------------------------
    if ([System.Environment]::OSVersion.Platform -ne 'Win32NT') {
        throw "binoc's Windows build only installs on Windows."
    }
    if (-not [System.Environment]::Is64BitOperatingSystem) {
        throw 'binoc ships as 64-bit only; this looks like a 32-bit Windows.'
    }
    if ($env:PROCESSOR_ARCHITECTURE -eq 'ARM64') {
        Write-Warning 'ARM64 Windows detected. binoc is x64-only, so it will run under emulation.'
    }
    ...
    # --- Resolve the release ---------------------------------------------------------------------------
    $release = Get-BinocRelease -Repo $Repo -Version $Version
    $tag     = $release.tag_name
    Write-Host "Installing binoc $tag" -ForegroundColor Cyan
    ...
    $work = Join-Path ([System.IO.Path]::GetTempPath()) ("binoc-install-" + [guid]::NewGuid().ToString('N').Substring(0, 12))
    ...
    try {
        ...
        # Invoke-WebRequest returns a WebResponseObject whose .Content is a byte[], NOT a string. Skip the
...
"@
        ...
        # --- Install -----------------------------------------------------------------------------------
        # Velopack's installer needs no admin rights: it installs to %LocalAppData%\Binoc, registers the
        # uninstaller and Start Menu shortcut, and launches binoc before exiting.
        #
        # So do NOT use `Start-Process -Wait`: that waits for the started process *and all its descendants*,
        # which includes the binoc window it just launched, so the wait never returns and the one-liner
        # hangs. Waiting on the Setup process's own handle instead returns as soon as the install is done.
        ...
        if (-not $proc.WaitForExit(10 * 60 * 1000)) {
            $keepWork = $true
            Write-Warning "The installer is still running after 10 minutes - leaving it to finish on its own. Delete $work once binoc has installed."
            return
        }
        if ($proc.ExitCode -ne 0) {
            throw "The binoc installer exited with code $($proc.ExitCode)."
        }
    }
    finally {
    ...
    Write-Host ''
    Write-Host "binoc $tag is installed and starting." -ForegroundColor Green
    Write-Host '  Drop an APK, AAB or IPA onto the window to analyse it.' -ForegroundColor DarkGray
}
...
function Get-BinocRelease {
    param([string] $Repo, [string] $Version)

    $headers = @{ 'Accept' = 'application/vnd.github+json'; 'User-Agent' = 'binoc-install.ps1' }
...
# makes a large download crawl on Windows PowerShell 5.1, and -OutFile there buffers the whole response
...
function Save-File {
    ...
    $req.UserAgent = 'binoc-install.ps1'
    $req.Timeout = 60000
...
Install-Binoc -Version $Version -Repo $Repo
